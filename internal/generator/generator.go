...
func New(file *importer.File) *Generator {
	...
	for _, pkg := range g.File.Imports {
		pkgPath := path.Join(importer.ProtoPath, pkg.GoOutPath)
		g.importGoPackage(pkgPath, pkg.GoPackage)
	}
...
func (g *Generator) header(buffer *bytes.Buffer) {
	...
	g.Pf("// source: %s\n", g.File.Proto.Filename)
	g.Pf("\n")
	g.Pf("package %s\n", g.File.Package.GoPackage)
	g.Pf("\n")
...
func (g *Generator) proto2() bool { return g.File.Version == 2 }
func (g *Generator) proto3() bool { return g.File.Version == 3 }
